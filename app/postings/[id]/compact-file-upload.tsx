...
export function CompactFileUpload({
  ...
  return (
    <div className="space-y-2">
      <label htmlFor={inputId} className="text-base font-medium text-ll-navy">
        {label}
      </label>
      <input
        ref={inputRef}
      ...
      >
        <span className="inline-flex items-center gap-2 text-base text-zinc-700">
          <span className="rounded-md border border-zinc-300 bg-white p-1 text-zinc-500">
            <Upload className="h-4 w-4" />
          </span>
          {selectedFileName || "Upload PDF"}
        </span>
        <span className="rounded-md bg-[#0f3441] px-2 py-1 text-sm font-semibold uppercase tracking-wide text-white">
          Choose file
        </span>
      </button>
